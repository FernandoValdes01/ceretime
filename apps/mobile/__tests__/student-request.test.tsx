...
async function openForm() {
  ...
  fireEvent.press(
    await screen.findByRole('button', { name: 'Entrar como Estudiante' }),
  );
  fireEvent.press(
...
describe('Formulario de solicitud del estudiante', () => {
  ...
  test('muestra errores visibles y permite corregirlos sin perder valores', async () => {
    ...
    fillRequiredFields();
    expect(
      screen.queryByText('Describe la necesidad que quieres abordar.'),
    ).not.toBeOnTheScreen();
    expect(
      screen.getByDisplayValue('Me cuesta leer los materiales del curso.'),
    ).toBeOnTheScreen();
    fireEvent.press(screen.getByRole('button', { name: 'Revisar formulario' }));
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText('Indica qué esperas del acompañamiento.'),
    ).toBeOnTheScreen();
  });
  ...
  test('permite varios apoyos y texto libre, sin exigir necesidades de acceso', async () => {
    ...
    fireEvent.press(
      screen.getByRole('checkbox', { name: 'Comunicación escrita' }),
    );
    expect(
      screen.getByRole('checkbox', { name: 'Comunicación escrita' }),
    ).not.toBeChecked();
    ...
    expect(
      screen.getByDisplayValue('Instrucciones por pasos.'),
    ).toBeOnTheScreen();
  });
  ...
  test('valida una franja opcional incompleta, inválida o invertida', async () => {
    ...
    expect(
      screen.getByText(
        'Escribe la hora inicial en formato HH:MM, por ejemplo 09:00.',
      ),
    ).toBeOnTheScreen();
    expect(
      screen.getByText(
        'Escribe la hora final en formato HH:MM, por ejemplo 13:00.',
      ),
    ).toBeOnTheScreen();
  ...
  test('el formulario no conserva el borrador después de cerrar sesión', async () => {
    ...
    await act(async () => router.back());
    fireEvent.press(
      await screen.findByRole('button', { name: 'Cambiar de rol' }),
    );
    fireEvent.press(
      await screen.findByRole('button', { name: 'Entrar como Estudiante' }),
    );
    ...
    expect(
      await screen.findByLabelText('¿Qué necesidad quieres abordar? *'),
    ).toHaveProp('value', '');
  });
  ...
  test.each(['Profesional', 'Practicante', 'Administrador'])(
    ...
    async (role) => {
      const navigation = renderRouter(appDirectory);
      fireEvent.press(
        await screen.findByRole('button', { name: `Entrar como ${role}` }),
      );
      await screen.findByText(`Inicio de ${role}`);
      await act(async () => router.push('/estudiante/nueva-solicitud'));
      await waitFor(() =>
        expect(navigation.getPathname()).toBe(`/${role.toLowerCase()}`),
      );
      expect(
        screen.queryByLabelText('¿Qué necesidad quieres abordar? *'),
      ).not.toBeOnTheScreen();
    },
  );
});
