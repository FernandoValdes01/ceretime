...
describe('Navegación principal', () => {
  ...
  test.each(roleCases)(
    ...
    async (label, href) => {
      ...
      await fireEvent.press(
        await screen.findByRole('button', { name: `Entrar como ${label}` }),
      );
      expect(await screen.findByText(`Inicio de ${label}`)).toBeOnTheScreen();
      expect(navigation.getPathname()).toBe(href);

      await fireEvent.press(
        screen.getByRole('button', { name: 'Cambiar de rol' }),
      );
      expect(
        await screen.findByText('Explora la aplicación'),
      ).toBeOnTheScreen();
      expect(navigation.getPathname()).toBe('/login');
  ...
  test.each(roleCases)(
    ...
    async (label, href) => {
      ...
      for (const [otherLabel, otherHref] of roleCases) {
        ...
        expect(
          screen.queryByText(`Inicio de ${otherLabel}`),
        ).not.toBeOnTheScreen();
      }
    },
  );
  ...
  test('una dirección inexistente ofrece volver al acceso', async () => {
    ...
    expect(
      await screen.findByRole('header', { name: 'Página no encontrada' }),
    ).toBeOnTheScreen();
    await fireEvent.press(
