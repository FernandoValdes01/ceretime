...
export function validateRequestForm(
  ...
  if (!values.needSummary.trim())
    errors.needSummary = 'Describe la necesidad que quieres abordar.';
  if (!values.expectedOutcome.trim())
    errors.expectedOutcome = 'Indica qué esperas del acompañamiento.';
  if (!values.modalityPreference)
    errors.modalityPreference = 'Selecciona una modalidad.';
  if (!values.preferredWeekdays.length)
    errors.preferredWeekdays = 'Selecciona al menos un día.';
  if (!values.preferredAccessibleInformationChannel.trim()) {
    errors.preferredAccessibleInformationChannel =
      'Indica cómo prefieres recibir información.';
  }
