...
function mount(
  ...
  function Probe() {
    return React.createElement(ProbeOutput, { value: useStudentArea(reader) });
  }
  ...
  return {
    renderer,
    getState: () => readProbeValue<StudentAreaState>(renderer),
  };
}
...
test("mounted hook transitions from loading to success and tracks empty collections independently", async () => {
  ...
  await act(async () => {
    read.resolve(await snapshot({ accompaniments: [accompaniment("accompaniment-1")] }));
    await read.promise;
  });
...
test("hides populated old data when the reader is replaced", async () => {
  ...
  function Probe() {
    return React.createElement(ProbeOutput, {
      value: useStudentArea(currentReader),
    });
  }
  ...
  const getState = () => readProbeValue<StudentAreaState>(renderer);
  await act(async () => {
    oldRead.resolve(await snapshot({ requests: [request("old")] }));
    await oldRead.promise;
  });
  expect(getState().snapshot?.requests[0]?.id).toBe("old");
  ...
  expect(getState().requests.data).toEqual([]);

  await act(async () => {
    newRead.resolve(await snapshot({ requests: [request("new")] }));
    await newRead.promise;
  });
  expect(getState().snapshot?.requests[0]?.id).toBe("new");
...
test("public aggregate hooks expose reload after rejection and retry independently", async () => {
  ...
  const getState = () => readProbeValue<AggregateState>(renderer);
  ...
  expect(getState().identity.status).toBe("error");
  expect(getState().requests.status).toBe("error");
  expect(getState().accompaniments.status).toBe("error");

  const reloads = [
    getState().identity.reload,
    getState().requests.reload,
    getState().accompaniments.reload,
  ];
  await act(async () => {
    reloads.forEach((reload) => reload());
  });
  expect(reads).toHaveLength(6);
  expect(getState().identity.status).toBe("loading");
  expect(getState().requests.status).toBe("loading");
  expect(getState().accompaniments.status).toBe("loading");
  ...
  expect(getState().identity.status).toBe("success");
  expect(getState().requests.status).toBe("empty");
  expect(getState().accompaniments.status).toBe("empty");
  act(() => renderer.unmount());
});
