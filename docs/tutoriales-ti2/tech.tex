...
La estructura de carpetas del monorepo no será la arquitectura completa. La
arquitectura lógica del sistema será una \textbf{arquitectura por capas},
aplicada de forma consistente en el backend y respetada por ambas
...
La dirección normal de una solicitud es \textbf{Presentación -> Aplicación ->
Dominio}. La infraestructura implementa las interfaces que necesitan la
aplicación y el dominio, por lo que puede depender de esas capas, pero las
reglas del negocio no dependen de Convex, React, Expo ni Better Auth.
...
TI2 será propietario técnico del backend compartido y de sus capas de
Aplicación, Dominio e Infraestructura. TI4 consumirá sus servicios y mantendrá
la Presentación Mobile desde la misma base de código.
...
web de TI2 y pertenecerá principalmente a la capa de Presentación.
...
Tampoco deberá contener reglas de negocio que deban cumplirse de la misma
forma en Web y Mobile.
...
La aplicación web funcionará principalmente como cliente de Convex. Vite es
una herramienta de construcción y desarrollo, no una capa de negocio.

Por esta razón no es necesario introducir un segundo framework full-stack con
una capa adicional de servidor.

La distribución del cliente Web será:
...
Convex no define por sí solo la arquitectura del sistema. Las funciones
públicas de Convex actuarán como adaptadores de la capa de Presentación: deben
recibir la solicitud, validar su forma y delegar en un caso de uso. No deberán
concentrar consultas, reglas de autorización y reglas de negocio en una única
función extensa.

\subsection{Tipos de funciones}

La interfaz pública del backend se implementará mediante:
...
Queries, mutations y actions pertenecen al borde de entrada del backend. El
caso de uso que invocan pertenece a la capa de Aplicación y las reglas que el
caso de uso debe respetar pertenecen al Dominio.
...
La mayor parte de la lógica de negocio no deberá permanecer directamente en
queries, mutations o actions. Estas funciones deben mantenerse delgadas para
que el mismo caso de uso pueda probarse y reutilizarse sin depender del
runtime de Convex.
...
El modelo de datos será administrado mediante Convex y pertenecerá a la capa de
Infraestructura.
...
entidades ni las reglas del Dominio. Cuando una regla de negocio necesite leer o
guardar información, la capa de Aplicación utilizará un puerto y la
Infraestructura lo implementará mediante Convex.
...
Better Auth administrará la autenticación de los usuarios y se ubicará en la
...
El adaptador de Better Auth podrá entregar la identidad autenticada al borde de
entrada. La Aplicación y el Dominio utilizarán esa identidad para evaluar
autorización y políticas del negocio, sin importar directamente módulos de
Better Auth.
...
Las reglas de autorización propias del negocio deberán implementarse en las
capas de Aplicación o Dominio, según corresponda, y no confiar exclusivamente
en que una pantalla esté oculta en el frontend.
...
React Native pertenecerá a la Presentación Mobile y Expo proporcionará el
entorno de ejecución para esa capa.
...
móvil y pertenecerá a la Presentación.
...
La lógica de negocio no deberá implementarse dentro del router. Las pantallas
deberán delegar las operaciones en la Aplicación del cliente o en el cliente
de infraestructura que invoque la API pública.
...
aplicaciones consumen la misma API pública. Las carpetas de las capas podrán
organizarse por funcionalidad dentro de cada capa cuando el producto crezca,
pero no se deberán mezclar responsabilidades solamente para evitar crear un
archivo nuevo.
...
Estos tipos pertenecen al borde entre Infraestructura y Presentación. No deben
convertirse en el modelo de Dominio ni hacer que las entidades del negocio
dependan de \texttt{convex/\_generated}. Cuando sea necesario compartir
contratos más estables, se podrán publicar desde \texttt{packages/shared/},
manteniendo separadas las reglas del negocio.
...
TI4 puede consumir las funciones del backend y utilizar sus tipos, pero no debe
modificar directamente las funciones Convex que no estén dentro de su
responsabilidad ni duplicar en Mobile los casos de uso del backend.
...
Las carpetas de las capas son límites de responsabilidad, no una obligación de
crear archivos vacíos. Una funcionalidad pequeña puede comenzar con pocos
archivos, siempre que cada archivo tenga una responsabilidad clara y conserve la
...
Después de crear el backend, se deberán establecer los límites de las capas
antes de agregar lógica de negocio:
...
No es necesario crear archivos vacíos en cada carpeta. El objetivo es decidir
primero dónde vive la responsabilidad y luego incorporar el código mínimo
...
La frontera técnica entre ambos equipos estará formada principalmente por la
API pública de la capa de Presentación del backend, los casos de uso que esa
API expone, el sistema de autenticación compartido y los paquetes internos que
sean consumidos por más de una aplicación.
...
Debe ser código TypeScript independiente de frameworks. No deberá recibir
directamente documentos de Convex ni tipos de navegación.
...
Aplicación y Dominio deciden \textbf{qué información puede obtener} el
usuario. Infraestructura solamente implementa la forma concreta de obtenerla.
...
\begin{enumerate}
  ...
  \item TI2 expone una función pública de Convex como punto de entrada del caso
    de uso.
...
Presentación. TypeScript se utiliza en todas las capas. Vite, Expo, EAS Build,
