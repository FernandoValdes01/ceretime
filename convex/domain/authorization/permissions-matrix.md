...
La habilitación de una cuenta Practicante y su asignación a un acompañamiento son decisiones distintas: una cuenta habilitada sin asignación activa no lee nada, y una asignación revocada deja de autorizar. La escritura usa la mutación interna guardada `internal.assignments.assign`, que exige que el llamante sea un Profesional con cuenta habilitada y vigente y rechaza una fila activa duplicada para la misma combinación de acompañamiento, usuario y rol; el recorte por acompañamiento (asignar solo en los propios) llega con el flujo público de RF-39. Las lecturas además deduplican por acompañamiento ante filas heredadas, y como cada tripla admite una sola fila activa, por rol no hay repetidos entre páginas. Una cuenta `@alu.uct.cl` solo opera como profesional de prueba dentro de acompañamientos explícitamente asignados y auditados (RN-25). La vista `minimized` excluye `accessNeeds` por tratarse de dato potencialmente sensible (Ley 21.719) y las notas internas quedan en una consulta separada solo para profesionales autorizados.
