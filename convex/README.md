...
La matriz automatizada cubre el flujo ya implementado sin ampliar funcionalidad: cuenta válida (`convex/session.test.ts` con `@alu.uct.cl` y `@uct.cl`, más `apps/web/src/application/session/institutional-login.test.ts` para el inicio por población y el retorno controlado), cuenta no autorizada (`convex/session.test.ts` con respuesta idéntica ante dominio externo y ausencia, más `reject_external_user.test.ts`, `minimal_identity.test.ts` y `auth-error.test.ts` para el mensaje genérico), sesión expirada (`convex/session.test.ts` con transición de `authenticated` a `unauthenticated` sin filtrar motivo, más el aviso visible en `apps/web/src/presentation/auth/AuthScreen.test.tsx`) y cierre (`convex/session.test.ts` con estado posterior sin datos mínimos, más `AuthScreen.test.tsx` con éxito, error resuelto y rechazo del `signOut` y los mensajes genéricos de `institutional-login.test.ts` para `signOutError`).

Para reproducir desde la raíz: `bun run test:convex`, `bun run test:web`, `bun run --cwd apps/web tsc -p ../../convex/tsconfig.json --noEmit`, `bun run --cwd apps/web tsc -p ../../convex/tsconfig.tests.json --noEmit`, `bun run lint`, `bun run format:check`, `bun run --cwd apps/web run lint` y `bun run --cwd apps/web run build`.

La evidencia manual con datos ficticios usa solo cuentas de prueba institucionales: una cuenta válida `@alu.uct.cl` ingresa y `getSessionState` responde `authenticated` con datos mínimos, una cuenta externa queda en `unauthenticated` con el mensaje genérico de ingreso, recargar sin sesión o con sesión vencida muestra “Tu sesión terminó. Vuelve a ingresar.” y tras `Cerrar sesión` la misma query responde `{"status":"unauthenticated"}` y la pantalla retorna al acceso institucional.
